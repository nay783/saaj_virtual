    ...
    <div className="relative w-full h-full flex flex-col flex-1 min-h-0 bg-slate-50 convo-core-page overflow-hidden">
      ...
      {isLoading && (
        <div
          className={`absolute inset-0 z-30 flex flex-col items-center justify-center p-6 space-y-4 animate-fadeIn ${
            isTonito ? "bg-blue-50/95" : "bg-pink-50/95"
          }`}
        >
          <div
            className={`w-12 h-12 rounded-full border-4 border-t-transparent animate-spin ${
              isTonito ? "border-tonito-500" : "border-manuela-500"
            }`}
          />
      ...
      {errorMessage && !isLoading && (
        <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-slate-50 p-6 text-center space-y-4 animate-fadeIn">
          <div className="p-3.5 rounded-2xl bg-red-50 text-red-600 shadow-2xs">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-500 max-w-xs mx-auto">{errorMessage}</p>
          </div>
          <div className="flex items-center gap-3 pt-2">
            <button
              onClick={() => setRetryCount((prev) => prev + 1)}
              className="inline-flex items-center gap-2 py-3 px-5 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs transition-all shadow-sm active-press"
            >
            ...
            {onBack && (
              <button
                onClick={onBack}
                className="inline-flex items-center gap-2 py-3 px-5 rounded-2xl bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold text-xs transition-all active-press"
              >
      ...
      <div className="convo-core-shell flex-1 min-h-0 min-w-0 w-full relative">
        <div
          ...
          className="w-full h-full min-h-0 overflow-hidden max-w-4xl mx-auto"
