...
export const AppointmentEmbedModal: React.FC<AppointmentEmbedModalProps> = ({
  ...
  if (!calLink) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 safe-top safe-bottom">
        <div className="bg-white rounded-3xl p-6 max-w-sm w-full text-center space-y-4 shadow-xl">
          <AlertCircle className="w-10 h-10 text-amber-500 mx-auto" />
          <h3 className="text-lg font-bold text-slate-900">Serviço indisponível</h3>
          <p className="text-xs text-slate-600">
            O agendamento de consultas presenciais está disponível atualmente apenas para Maxixe e Massinga.
          </p>
          <button
            onClick={onClose}
            className="w-full py-3 rounded-2xl bg-slate-900 text-white font-semibold text-xs"
          >
            Fechar
          </button>
        </div>
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-slate-50 safe-top safe-bottom animate-fadeIn">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b border-slate-200">
        <div className="flex items-center gap-2">
          <div className="p-2 rounded-xl bg-blue-50 text-tonito-600">
      ...
      <div className="bg-white border-t border-slate-200 px-4 py-2.5 text-center text-[11px] text-slate-400">
